...
class Piece 
  ...
  def initialize (point_array, board)
    @all_rotations = point_array # here, point_array refers to all posible rotations of a random piece
    @rotation_index = (0..(@all_rotations.size-1)).to_a.sample
    @color = All_Colors.sample
    @base_position = [5, 0] # [column, row]
    @board = board # board is Board's object
    @moved = true
  end
  ...
  def move (delta_x, delta_y, delta_rotation)
    ...
    moved = true
    #for delta_rotation, -1 means clockwise 90, 1 means counterclockwise 90
  ...
  def self.rotations (point_array) #here, point_array refers to one of the four rotations of a piece
    rotate1 = point_array.map {|x,y| [-y,x]}  #counterclockwise 90 = colckwise 270
    rotate2 = point_array.map {|x,y| [-x,-y]} #180
    rotate3 = point_array.map {|x,y| [y,-x]}  #clockwise 90 = counterclockwise 270
...
class Board
  ...
  def store_current
    locations = @current_block.current_rotation # a particular rotation of a piece
    displacement = @current_block.position # base position
    (0..3).each{|index| 
...
class Tetris
  ...
  def key_bindings  
    @root.bind('n', proc {self.new_game}) #by 'proc {method}', we transfer this method to a callable object, which will only excuted when called. 
