...
class LineSegment < GeometryValue
  ...
  def shift(dx, dy)
    LineSegment.new(x1+dx, y1+dy, x2+dx, x2+dy)
  end
...
class Intersect < GeometryExpression
  ...
  def preprocess_prog #e could either be geovalue or geoexpression
...
class Shift < GeometryExpression
  ...
  def eval_prog env
    v_origin = @e.eval_prog(env)
    v_origin.shift(@dx, @dy)
  end
end
